...
## 구현 중 확인된 사실 (PR 1 실행 기록)

### 양식 제약 — 과업분석표 "직무" 열의 세로 병합

정본 로드맵 v2 의 과업·워크플로우 분석표(T16)에서 **직무 열 (1,0) 셀이 `rowSpan=2` 로 세로 병합**되어 있다(같은 직무에 과업 2개를 묶는 양식 예시 구조). 따라서 **과업 0·1 은 같은 직무 셀을 공유**하며, 과업 1 의 직무는 별도 출력되지 않는다.
SSOT R-10 에 `skip_items: [1]` 로 명시해 마커를 심지 않으며(심으면 앞 값을 덮어써 조용히 유실됨), `verify_hwpx_placeholders.py` 가 이 정합을 CI 에서 보증한다.

### PBL 템플릿은 v1 유지 (PR 3 까지)

PBL 코드는 아직 좌표 방식이므로, `templates/hwpx/pbl.hwpx` 는 **v1 로 되돌려 두었다**. PR 1 만 머지해도 PBL HWPX 가 깨지지 않는다. 신규 PBL 정본은 `docs/references/` 에 보관되어 있으며 PR 3 에서 마커 전환과 함께 적용한다.

### 성능 — 마커 전환의 부수 효과

로드맵 HWPX 생성이 **4.1초 → 0.1초** 로 단축됐다(표 전수 순회 + 좌표 탐색 제거).
⚠️ 반면 **PBL 생성은 여전히 43초** 다 — v1 양식에서도 동일했던 **기존 성능 문제**로 이번 변경과 무관하다. pytest 전체가 24분 걸리는 주원인이기도 하다.

**프로파일 결과 (PR 3 대비):** 병목은 python-hwpx 의 `document.py::_build_cell_grid` 다. 셀 접근마다 표 전체 그리드를 재구성(캐시 없음)하는데, PBL 은 표 54개 + `_replace_in_all_runs`(체크박스 치환 등)가 **모든 표의 모든 셀을 반복 순회**하면서 매번 그리드를 다시 만들어 `_build_cell_grid` 20만 회, 하위 `_addr_element`/`_span_element` 가 **940만 회** 호출된다(총 60초). → **로드맵처럼 마커 방식으로 전환하면 셀을 1회만 순회하므로 자연 해소**될 전망이다(로드맵 4.1→0.1초로 입증). PR 3 의 PBL 마커 전환 시 측정 확인.

### 부수적으로 고친 회귀 (v1 부터 존재하던 버그)

결과 화면 Ⅰ-3 "수립 주요내용 요약" 인라인 편집이 `extractRoadmapFieldsFromPayload` 에 `main_content` 키가 없어 **저장이 조용히 유실**되고 있었다(v1 부터 "이월" 주석으로 방치). `hydrateOutcomeSummary()` 로 현재 값을 읽어 3필드 완전체로 병합해 전달하도록 수정.
