...
> **상태(2026-08-02, 세션 15): 6건 전부 완료** — `P5`(PR #135) · `P6`(PR #138 — 2차 원자화 RPC 는 선택 과제로 잔존) · `P8`(PR #154) · `P9`(**PR #125 로 2026-06-06 에 이미 완료됐음을 세션 13 실측으로 확인** — "미착수" 기록이 오류였다. #125 이후 생긴 잔여 불일치 6건은 PR #166 으로 정정, 아래 P9 반전 블록) · `P4`(**PR #127 로 2026-06-07 에 이미 완료됐음을 세션 14 실측으로 확인** — 같은 유형의 10번째 정정 사례, 아래 P4 반전 블록) · `P7`(1단계+분할안 B~F PR #155~#158 · lib 역전+가드 PR #166 · **단계 5 PR #170·#171 — 아래 단계 5 완료 블록**). **P7-layer-inversion 종결: components·hooks 의 `@/app` 역참조 0건 + eslint 가드 예외 목록 소멸.**
...
**권장 순서:** ① `P9`(스킬 문서, 즉시) → ~~② `P5`~~ → ~~③ `P6`~~ → ~~④ `P4`(editPBLV2 분해)~~ → ~~⑤ `P8`~~ → ⑥ `P7`(계층 역전, 노력 높음·범위 큼·마지막). 단 B묶음(P5·P6)은 동작이 바뀌므로 각각 사용자 승인 후 착수. (P9·P4 는 착수 시점에 이미 완료 상태였음이 뒤늦게 확인됨 — 각 반전 블록 참조.)
...
> - **`P6` 완료 (PR #138, 2026-07-29)** — 본문이 지목한 3곳이 아니라 **8곳**이었다. 누락분은 인터뷰 저장 3곳(`interview/actions.ts`)과 진단 완료 2곳(`assessment/actions.ts`·`ops/.../crud.ts`). 인터뷰 3곳은 로깅뿐 아니라 **알림 오발송까지** 고쳤다 — `statusTransitioned` 를 전이 성공 시에만 세워, 전이 실패 시 "인터뷰 완료" 알림이 나가지 않는다. 진단 2곳은 `.eq('status','NEW')` 멱등 가드가 있어 0행 매치가 정상이므로 error 만 검사한다. 부록이 예고한 2차(반환 타입 변경·RPC 원자화)는 **PR #172 에서 완료**(아래 P6 절의 2차 완료 블록).
...
>
> 🔄 **분할안 D 완료 (PR #157, 2026-07-31)** — `DeleteAccountSection`(+테스트)을 `dashboard/settings/_components` 로, `PublicSelfAssessmentForm`(+테스트)을 **신설** `assessment/[token]/_components` 로((dashboard) 그룹 밖 최초 `_components`), 역참조 0 인 `TrackFilter`·`ScopeFilter`(+테스트)는 **순수 응집도 이동**(사용자 위임 판단 — 소비자가 `GalleryContent.tsx` 한 파일 2줄뿐·외부 mock 0·e2e 는 testid 만)으로 gallery `_components` 로 — 총 git mv **12파일**, 소비자 갱신 4줄 + D-2 자체 import·mock 상대화 2줄. `(auth)/actions` 는 크로스 라우트그룹이라 **절대 유지**(UserManagementTable 선례), `assessment/actions` 는 같은 라우트 서브트리라 **`'../../actions'` 상대화**(AssessmentTokenSection 선례 — 테스트 mock 도 동일 문자열로 통일). `components/assessment` 는 **형제 없음 실측**(폼+테스트 2파일뿐) → 디렉터리 소멸, 분할안의 "형제 응집도 점검" 항목 종결. 잔여 역참조 **14 → 12파일**(TrackFilter·ScopeFilter 는 원래 역참조 0 이라 미차감). coverage **91.3/84.85/87.2/92.38 — #156 과 완전 동일**(이동 분모 중립 실측. 단 `[token]/PublicAssessmentClient.tsx`·`page.tsx` 는 coverage include 밖이라 `_components` 로 끌어들이면 테스트 0 파일이 분모에 신규 진입 — 이동 대상에서 의도적 제외). ⚠️ **결함 주입 실측: settings `page.test.tsx` 의 mock 을 옛 절대경로로 역치환해도 7건 전부 green** — 이 테스트는 render 없이 `await SettingsPage()` 만 호출해 실물 컴포넌트가 로드돼도 실패하지 않는다. 즉 이 mock 실효는 **typecheck·lint·test 3중을 전부 통과하는 완전 조용한 실효**(B 의 "19건 중 2건 실패"와 다른 유형 — render 하는 테스트만 주입을 잡는다). **grep 전수가 유일한 안전망**이므로 이동 PR 완료 판정에 옛 경로 grep 0건을 필수 포함할 것. D-2·D-3 은 주입 생략(외부 mock 0 + SUT 상대 import 라 이동 누락 시 resolve 실패로 즉시 검출).
>
> 📌 **분할안 E 실측 완료·구현 이월 (2026-07-31, 세션 11 탐사)** — 구현은 다음 세션 → ✅ **PR #158 로 구현 완료**(아래 E+F 완료 블록 참조, 정정 8번째 사례 포함). 실측: ① **4종(`ConsultantSelector`·`ManualAssignmentForm`·`SelfAssessmentForm`·`RecommendationResults`) 전부 `ops/projects/[id]/page.tsx` 단일 수렴**(ConsultantSelector→ManualAssignmentForm→AssignmentTabSection→page / SelfAssessmentForm→CollapsibleDirectInput→page / RecommendationResults→AssignmentTabSection→page) — 단계 5 이월 대상 0건, 전부 `ops/projects/[id]/_components` 이동 성립 ② **본문 위치표·부록의 "RecommendationResults 는 배럴+AssignmentTabSection 다중 소비라 단계 4 제외" 전제가 틀렸다(7번째 정정 사례)** — `AssignmentTabSection.tsx` 는 `./assignment/RecommendationResults` **직접 경로** import 이고 `assignment/index.ts` 의 해당 재수출은 **importer 0 인 dead 재수출**(export 는 소비가 아니다) ③ **4종만 옮기면 새 역전 2건이 생긴다** — 부모 `AssignmentTabSection`·`CollapsibleDirectInput`(현재 `@/app` 역참조 0)이 components→app 역참조를 새로 얻음 → **부모 2종 동반 이동 필수**(1단계 UploadProgress·AttachmentList 와 동일 유형의 함정). CollapsibleDirectInput 은 `__tests__/` 테스트+스냅샷 파일 동반 ④ **`self-assessment/` 디렉터리는 잔류 확정** — `src/lib/services/calculate-scores.ts:1` 이 `MAX_SCALE` 을 import 하므로 라우트로 옮기면 **lib 가드 위반으로 빌드 차단** + 5모듈 다중 소비(assessment·consultant·ops/templates 라우트 + lib). 이 **lib→components 역전**은 lib 가드(`@/app/**` 만 차단)에 안 걸린 채 존재 — F 또는 별도 항목에서 처리 검토 ⑤ `assignment/` 하위(배럴·utils·constants·useAssignmentMatching 포함)는 외부 소비자가 `ops/projects/[id]` 라우트뿐(`page.tsx` 의 `assignment/utils` type·`ProjectClosureControls` 의 `ReasonLengthHint`·`constants`)이라 **통째 이동/잔류 양쪽 성립** — E 착수 시 결정(파일 수가 조사 간 12 vs 17 로 어긋나 착수 시 재실측) ⑥ mock 실효 구멍: 전량 동반 이동이면 상대 mock 전부 유효 — 유일 위험은 **부분 이동**(상대 mock 파손). e2e 는 testid(`consultant-list-item`)·텍스트만 참조라 이동 영향 0 ⑦ 부수 발견(E 에서 삭제 후보): `assignment/index.ts` dead 재수출 4건(ReasonLengthHint·SelectableCard·RecommendationResults·useAssignmentMatching) + 프로덕션 소비자 0 아이콘 2개(SpinnerIcon·RefreshIcon).

> ✅ **분할안 E+F 완료 (PR #158, 2026-08-01, 세션 12)** — 한 PR 순차 완결(사용자 결정). 화면·기능 변화 0.
> **실행:** ① E — 조립 4종 + 부모 2종(`__tests__/`·스냅샷 동반) + `assignment/` 17파일 = **git mv 28파일** → `ops/projects/[id]/_components/`(내부 상대 import·mock 은 구조 보존으로 무변경 유효). actions 상대화 14곳(`'../../actions'`, `assignment/` 하위는 `'../../../actions'` — 소스·vi.mock 동일 문자열), `'./self-assessment'` 절대화 2곳, `page.tsx` 3줄·`ProjectClosureControls.tsx` 2줄 갱신 ② dead 정리(사용자 승인) — 배럴 재수출 30→**실소비 8심볼**(dead 22개 제거), 심볼 완전 dead `SpinnerIcon`·`RefreshIcon` 삭제(Icons.tsx **부분** 삭제 — 살아있는 3개 보존, 자기 검증 테스트 −7건, ConsultantSelector.test 의 dead mock 항목 동반 제거) ③ F — `src/components/**`+**`src/hooks/**`** 의 `@/app/**`정적 import 를 error 차단(PR #155 lib 가드 동형,`eslint.config.mjs`), 기존 위반 **9파일 예외 동결**(파일 명시 열거 — 디렉터리 글롭 금지 + "단계 5 진행분만큼 감소" 주석), 테스트 예외 블록 없음. **위반 주입 검증**: components·hooks 각 1건 주입 → lint error 검출 → Edit 역치환 원복.
> **실측 정정(8번째 사례):** ① 위 E 실측 ①의 "4종 page 단일 수렴"은 부정확 — page 직접 import 는 부모 2종뿐이고 조립 3종은 ops 밖 소비자 0(부모 경유 체인). 이동 성립 결론 자체는 유지 ② dead 재수출 "4건"은 **22개**(배럴 30 export 중 실소비 8 — knip 은 이번에도 0건 보고, grep 전수가 판정) ③ self-assessment "5모듈" 목록 오기 — `SelfAssessmentForm` 은 외부가 아니라 이동 대상 본인이고 `CollapsibleDirectInput.tsx:9` 가 누락(실제 외부 소비 5파일 = assessment 라우트 2·consultant 1·ops/templates 1·lib 1) ④ `ops/projects/actions` 는 파일이 아니라 **디렉터리 배럴**(15파일) ⑤ `assignment/` **17파일 확정**(12 vs 17 논쟁 종결) — 통째 이동 근거: 6종 이동 후 잔류 시 소비자 전원 `[id]` 라우트뿐인 "한 라우트 전용 공용 디렉터리"가 됨 ⑥ **신규 발견: `src/hooks/useCommandPalette.ts:6` 이 9번째 역전**(`search/actions`) — components 만 막으면 hooks 경유 유입이 계속돼 사용자 승인으로 가드 files 에 hooks 동봉 ⑦ 부모 2종은 애초에 위반자 아님(`@/app` import 0) — "안 옮기면 새 역전 2건" 명분은 실측대로 옳았음 ⑧ F 테스트 예외 불필요 실측 — `no-restricted-imports` 는 정적 import 만 검사(vi.mock·동적 import 불검출, ESLint 소스 확인)하고 걸리던 테스트 3파일이 전부 E 이동 대상이라 E 후 위반 0건(예외 블록을 넣으면 오히려 구멍). `@/app/**`글롭의 라우트 그룹 괄호 매칭도 이번에 최초로 주입 실측 검증.
**잔여 lib→components 역전 1건 처리 옵션(이번 구현 금지 — 단계 5 또는 별도 항목에서 결정):**`src/lib/services/calculate-scores.ts:1`→`self-assessment/constants.ts:9`의`MAX_SCALE`(순수 상수 파일, JSX 없음). **A안(권장):** `MAX_SCALE`만`src/lib/constants/`로 내리고`constants.ts`가 re-export — 갱신 1파일(calculate-scores), 나머지 소비자·배럴·vi.mock 전부 무변경. **B안(배제):**`constants.ts`통째 lib 이동 —`:1`의`import type { Question } from './types'`때문에`types.ts`동반 필요 → 소비자 6+ 연쇄 갱신으로 같은 성격의 부채 재생산. **단계 5 잔여 = F 예외 목록과 동일 9파일**(셸 3 + ProfileForm·ProfilePageClient + LikeButton·ShareToggle + AttachmentList + useCommandPalette).
→ ✅ **A안 구현 + 재발 가드 완료(PR #166, 세션 13):** 신설`src/lib/constants/self-assessment.ts`·`constants.ts`는 re-export · 갱신 1파일 전제가 실측대로 성립(값 소비자 전부 배럴 경유,`constants`직접 경로 소비 0, vi.mock 2건은 배럴 factory 라 무감). 주입 검증(5→6, calculate-scores.test 7건 실패 검출) 후 원복. lib 가드 patterns 에`@/components/**`·`@/hooks/**` 추가(위반 0 시점 도입 — 예외 목록 없음, 주입 2건 검출 검증). coverage 분모 +1(신설 .ts)에도 4지표 하락 없음(91.19/85.18/86.8/92.31).
검증: 커밋별 validate GREEN **394파일 6725→6718건**(아이콘 테스트 −7 외 증감 0) · coverage **91.3/84.84/87.19/92.38**(임계 89/83/86/89, 이동 분모 중립) · build 0 · grep 전수(옛 경로 5종·`@/components/ops/assignment`·삭제 심볼 0건 / `@/components/ops/self-assessment`9파일 보존 / mock 짝`'./index'`소스·mock 동일 유지) ·`git diff`32파일 허용 목록 일치(부모 2종 diff 는 계획된 import 갱신+prettier 정규화뿐) · 로컬 E2E`project-lifecycle`·`consultant-reassignment`·`ops/projects` **24통과/2skip**(LLM 환경 의존 graceful skip — spec 설계, 변경 무관).

> ✅ **단계 5 완료 (PR #170 순수 이동 + PR #171 주입 전환, 2026-08-02, 세션 15)** — F 예외 9파일 전건 해소, **예외 블록 소멸**(components·hooks 가드 전면 적용). 화면·기능 변화 0.
> **실행:** ① 순수 이동 7건(#170) — 셸 4종(`Navigation`·`MessageIcon`·`NotificationBell`·`useCommandPalette`)+`ProfilePageClient` → `(dashboard)/_components/` **신설**(라우트 그룹 직하 최초 사례), `LikeButton` → `gallery/_components`, `ShareToggle` → `consultant/projects/[id]/_components`(`gallery/actions` import 는 app→app 정방향 잔류 — UserManagementTable 선례). git mv 14파일 + 소비자 8곳·테스트 mock 8곳 갱신, **`@/app` 액션 mock 9건은 전부 불변**(액션은 안 움직임 — 이동안의 결정적 안전 이점) ② 주입 2건(#171) — `ProfileForm` 은 `submitAction` **필수화**(내부 `profile ?` 삼항 제거 → 단일 호출. register/page 는 기주입이라 무변경, `ProfilePageClient` 만 `profile ? updateConsultantProfile : saveConsultantProfile` 주입 — 동일 profile 값 기준 등가), `AttachmentList` 는 `onDownload` 대칭으로 `onDelete` 주입(**판별 타입** — `editable: true` 일 때만 필수라 열람 래퍼 NoticeAttachmentDownloader 는 무변경, NoticeForm 만 주입) ③ eslint 예외 9→2(#170)→**0**(#171, 블록 통째 제거).
> **실측 정정(11번째 사례 — 본문·부록 대비):** 본문 단계 5·부록 P7 절의 **"셸 3 은 layout.tsx 공유라 단일 `_components` 이동 불가 → 주입으로만 해소" 서술은 거짓**(세션 14 재조사를 세션 15 착수 시 Explore 3에이전트로 재반증 — 이번엔 HANDOFF 실측 정본이 전 항목 정확했다). Navigation 의 비테스트 소비자는 `(dashboard)/layout.tsx:3` **1곳뿐**(layout 11개 중 1개), 나머지 3종은 Navigation 전용 — 9건 중 **7건이 순수 이동으로 해소**되고 진짜 주입 필요는 크로스 그룹 `ProfileForm` + 형제 라우트 교차 `AttachmentList` 2건뿐. "주입 전환 시 mock 조용한 실효" 우려도 실측상 **Navigation 1건만 해당**(나머지 8건은 hoisted 로컬 mock 단언이라 시끄럽게 RED — D 의 "render 없는 테스트" 유형 없음).
> **검증:** 특성화 선작성(submitAction 주입 경로 — 기존 유닛 커버 0건이었음) · 결함 주입 2회(이동: Navigation.test 부품 mock 옛 경로 역치환 → **61/70 RED** / 주입: submitAction 무시 역치환 → **11/62 RED**, 원복 모두 Edit 역치환) · 옛 경로 7종+액션 심볼 grep 전수 0건(src/+e2e/) · validate 395파일 **6735→6738건**(특성화 +3 외 증감 0) · build 0 · coverage **91.36/85.21/87.25/92.45**(임계 89/83/86/89 — `useCommandPalette.ts` 는 coverage include `_components/**/*.tsx` 밖 이탈, #155 util 관례 동형) · #170 PR CI 전 check pass + **E2E 아티팩트 실측 422/14/0·flaky 0**(baseline 동일 — expected 감소 없음).
...
> ✅ **실측 반전(9번째 정정 사례 — 이번엔 방향이 반대): 이 항목은 PR #125(`3fd527a6`, 2026-06-06)로 이미 완료돼 있었다**(세션 13, 2026-08-01 확인). 본 문서 상태 헤더·HANDOFF 가 "미착수"로 기록해 왔으나, 착수 실측에서 현행 SKILL.md(150줄)에 아래 처방이 전부 반영돼 있음을 확인 — 리터럴 `.includes` 0건·`verifyProjectAccess` 0건·표준 헬퍼 예시(A-1/A-2)·한눈표 존재. **아래 접근법 1~5 는 무효(기적용)**, 접근법 4 의 `if ('error' in access)` 관용구 처방은 부록 정정대로 틀렸고 현행 문서가 이미 올바르다(`!== true`). 세션 13 이 실제 수정한 것은 **#125 이후 코드 진화로 생긴 별도 잔여 불일치 6건**(PR #166): ① 5단계 1항 표준을 requireAuth/requireAuthWithRole 로('use server' 36개 중 30개, `status==='ACTIVE'` 검사 병기) ② 패턴 B/C 예시 변수 `profile.role`→`role` ③ 한눈표 시그니처에 행정 종결 options(`blockClosed`/`allowClosed` — 기본 동작이 서로 반대) ④ `requireAuth` 행·`PROJECT_CLOSED_ERROR` 보강 ⑤ `(auth)/actions.ts`→`(auth)/actions/` 디렉터리 ⑥ "ActionResult 로컬 재정의 다수/마이그 예정"→완료(잔존 0건). 아래 위치표·수치도 다수 stale: auth-helpers 라인(86→96·112→130·143→166), "20+ 콜사이트"→**44곳/14파일**, pbl/actions.ts:204→226·252, interview/actions.ts verifyProjectAccess 48→50. **교훈: "미착수" 기록도 착수 전 반증 대상 — 완료를 놓친 채 계획만 남은 항목은 처방을 그대로 실행하면 정확해진 현행 문서를 되레 훼손한다.**
...
> ✅ **실측 반전(10번째 정정 사례 — P9 와 동형): 이 항목은 PR #127(`2875786a`, 2026-06-07)로 이미 완료돼 있었다**(세션 14, 2026-08-01 확인). 본 문서 상태 헤더·HANDOFF 가 두 달간 "미착수"로 기록해 왔으나(같은 6월 웨이브 #123 P1·P3·P2 → #125 P9 → #127 P4 중 P9 반전만 세션 13 이 발견), 착수 실측에서 본문 처방·부록 단서가 **전부 현행 코드에 반영돼 있음**을 확인: ① `mergeTrainingEnv`/`mergeProblemDefinitionSheet` 가 `services/interview/converters.ts:605/616` 에 실재 — `stripNullish(patch)` 로 undefined/null 값 키를 제거해 `??` 시맨틱 보존, **기본값 충전·검증은 하류 `PBLInterviewSchema.partial().safeParse` 단일점에 위임**(부록 P4 보강 2안 "더 안전한 무동작변화 대안"의 형태 — 헬퍼 내부 parse 없음 → 에러 메시지 경로 불변, deepMerge 미채택, nested 통째 교체, docstring 의미 계약 고정) ② editPBLV2(현 921-1172)는 헬퍼 호출(:1081/:1092)로 교체 완료, 하드코딩 기본값 블록 삭제 ③ 부록이 요구한 발산 케이스는 **특성화 26건으로 기박제** — actions-v2.test.ts:687-930 "editPBLV2 병합 특성화 — P4 동작 보존 안전망" 15건(explicit undefined/null·빈 문자열·빈 patch·default 전수·nested 통째교체 후 default 리셋·구체 에러 메시지 경로) + converters.test.ts:640-737 11건. update 페이로드 실캡처 방식이라 실효성 있음 — **세션 14 결함 주입 재검증: `stripNullish` 의 null 제거 무력화 → 두 계층 정확 5건 RED(converters 3·actions-v2 2) → 원복 84/84 GREEN**. **결정적 증거: 본 문서의 위치표·부록 인용 라인(861-1168·1022-1074·310-345/348-381 등)이 `2875786a^`(P4 직전 트리)와 1:1 일치 — 이 문서 자체가 P4 직전 스냅샷에서 작성됐다.** 아래 접근법 1~6·검증·리스크는 무효(기적용). **세션 14 실측 정정 3건:** ⓐ "trainingEnv 16필드"는 **15필드** 오기(스키마 전수 실측 — 코드 주석 3곳 `actions.ts`·`converters.ts`·`actions-v2.test.ts` 도 세션 14 에서 함께 정정, 스키마 default ↔ pre-P4 하드코딩은 15+4필드 전수 1:1 일치·값 불일치 0) ⓑ editPBLV2 를 행사하는 **E2E 는 0건**(`grep editPBLV2 e2e/` 0 — 이 경로의 회귀 안전망은 위 단위 26건이 유일) ⓒ **이론상 미세 발산 1건 잔존(기록만):** current(저장값) 쪽 필드가 explicit null 이면 pre-P4 `??` 는 기본값으로 조용히 치유했으나 현행은 하류 safeParse 실패로 편집 저장이 거부됨 — 도달 경로 미확인(스키마 경유 저장은 null 을 만들지 않음)·#127 이후 2개월 무증상. 수정하면 현행 운영 동작 변경이므로 **착수 금지선 — 실증 사례가 나타날 때 별도 설계+사용자 승인으로만**.
...
> ✅ **2차(원자화) 완료 (PR #172, 2026-08-02)** — 1차(PR #138)의 error 로깅에서 더 나아가, 산출물 저장과
