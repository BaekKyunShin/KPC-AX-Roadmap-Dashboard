...
### ✅ #019 연결이 끊겼다 붙을 때 좌측 대화 목록이 갱신되지 않음 — 해결 완료 (2026-07-31, **처방 교체**)

> **✅ 완료 (2026-07-31)** — 아래 원래 처방(`useMessageRealtime` 훅 통합)은 **채택하지 않았다.**
> 재검증에서 그 처방의 개선 메커니즘이 코드 구조상 성립하지 않음이 드러났기 때문이다(바로 아래 블록).
>
> **실제로 한 것:** `MessagesClient.tsx` 의 `realtimeActiveRef` 를 채널별로 분리
> (`threadRealtimeActiveRef` / `listRealtimeActiveRef`)하고, **목록 갱신 폴백을 대화 선택과 무관한
> 별도 interval 로 분리**했다(`LIST_POLL_MS`). `MessageIcon`·`NotificationBell` 은 손대지 않았다.
>
> **왜 이게 맞는 수정인가** — 사용자가 겪던 것은 "재시도가 느린 것"이 아니라 **좌측 목록이 멈추는 것**이었다:
> ① 목록 채널만 끊기면 폴백이 통째로 skip 돼 다른 대화의 새 메시지가 목록에 안 뜨고,
> ② 대화를 하나도 열지 않으면 목록 갱신 폴백이 아예 등록되지 않았다.
>
> **안전망:** 기존 realtime 테스트는 mock 이 콜백을 버려 **실질 0건**이었다. `NotificationBell.test.tsx`
> 방식을 채널 2개용으로 확장해 모킹을 다시 만들고, 특성화 6건 + 신규 5건을 추가했다.
> 결함 주입 4종(플래그 통합 · 폴백 미등록 · skip 가드 무력화 · 본인 메시지 필터 제거)이 모두 검출됐다.
>
> **검증:** validate 397파일 **6775건** · build 통과 · 로컬 E2E `messages`·`messages-realtime`·
> `scroll-ux` 전체 **32 통과 / 5 skip**(모두 기존 skip).
