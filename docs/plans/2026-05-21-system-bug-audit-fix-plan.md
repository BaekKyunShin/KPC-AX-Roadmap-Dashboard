...
### #008 새 알림이 도착해도 종 아이콘이 최대 30초 늦게 갱신
...
### #009 메시지 새 대화 생성이 실패해도 다이얼로그 그대로 [P3 격하]
...
### #010 메시지 초기 로드가 실패하면 "대화 없음" 화면과 구분 불가
...
### #011 PBL 갤러리 상세에 공유 토글이 없음 (로드맵과 비대칭)
...
### #012 알림 클릭했는데 아무 변화 없음 (이동 대상 누락 알림)
...
### #015 PBL 트랙 프로젝트가 /roadmap 진입 시 사유 없이 강제 이동 [P3 격하]
...
### #016 누적된 알림이 많을수록 알림 페이지·종 아이콘이 느려짐
...
### #017 감사로그 액터·대상 필터 적용 시 결과 표시 지연
...
### #019 모바일 네트워크 전환 시 메시지 실시간 복구가 느림
