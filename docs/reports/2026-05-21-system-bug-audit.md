...
| 2026-07-30 | 알림 실시간·링크·인덱스 (PR #147)    | #008 · #012 · #016 RESOLVED · #020 결함 아님 판정          |
| 2026-07-30 | PBL 공유 + 메시지 안내 + 감사 인덱스 | #011 · #010 · #009 · #017 RESOLVED (+ #011 대상 화면 정정) |
...
| #008 | **P2** ★★★★  | 알림          | 헤더 > 알림 벨                             | 알림 벨에 Realtime 구독 없어 새 알림이 최대 30초 지연                              | 🟢 RESOLVED                |
| #009 | **P3** ★★    | 메시지        | 헤더 > 메시지 > "새 메시지"                | 새 대화 생성 실패 시 다이얼로그가 닫히지 않음 (재검증 후 P3 격하 — 의도된 UX 가능) | 🟢 RESOLVED                |
| #010 | **P2** ★★★   | 메시지        | 헤더 > 메시지                              | 초기 로드 실패 시 빈 상태와 구분 불가 — 에러 토스트·재시도 부재                    | 🟢 RESOLVED                |
| #011 | **P2** ★★★   | 갤러리 공유   | 컨설턴트 > 프로젝트 > PBL 결과             | PBL 결과 화면에 공유 토글 누락 — 갤러리에 올릴 수단 자체가 없음 (대상 화면 정정)   | 🟢 RESOLVED                |
| #012 | **P2** ★★★   | 알림          | 헤더 > 알림 벨 > 알림 클릭                 | 알림에 이동 대상 링크가 없으면 클릭해도 아무 변화 없음                             | 🟢 RESOLVED                |
| #013 | **P2** ★★★   | 토스트        | 컨설턴트 > 프로젝트 > 기업 정보 수정       | 토스트 호출 패턴이 한 컴포넌트만 다름 (정책 변경 시 누락 위험)                     | 🟢 RESOLVED                |
...
- **상태 변경 (2026-07-30, PR #147):** 🔴 OPEN → 🟢 **RESOLVED**.
  `NotificationBell.tsx` 에 `MessageIcon` 패턴(인증 대기 → INSERT 구독 → 지수 백오프 재시도 →
  소진 시 polling)을 이식했다.
  ⚠️ **계획서보다 범위가 넓었다**: `notifications` 가 `supabase_realtime` **publication 에 없어서**
  (019 가 `messages` 만 추가) 구독 코드만으로는 이벤트가 오지 않는다 →
  마이그 `078_enable_realtime_notifications.sql` 이 함께 필요했다.
  30초 polling 은 **제거하지 않고 폴백으로 유지**한다(구독 성립 시 중단, 실패 시 계속).
  실제 이벤트 전달·타인 알림 미수신은 로컬에서 스크립트로 실측 확인했다.
- **기술 근거:**
...
- **상태 변경:** 🔴 OPEN → 🟢 RESOLVED (2026-07-30) — 실패 사유를 다이얼로그 안에 `role="alert"` 로 남기고(서버가 준 메시지 + "다른 사용자를 선택해 다시 시도해주세요." + 닫기 버튼), 토스트는 기존대로 함께 띄운다. **다이얼로그를 닫지 않는 정책은 그대로**(재검증에서 의도된 재선택 UX 로 판정된 부분) — 그 사실을 특성화 테스트로 고정했다. 단위 테스트 6건 신규.
...
- **상태 변경:** 🔴 OPEN → 🟢 RESOLVED (2026-07-30) — `loadError` state 로 실패와 빈 상태를 분리하고 "메시지를 불러오지 못했습니다 / 잠시 후 다시 시도해주세요. 계속되면 운영팀에 문의해주세요. + [다시 시도]" 화면을 렌더한다(운영관리 > 사용자 관리의 기존 에러 UI 와 동일 톤).
  ⚠️ **문서가 지목한 1곳이 아니라 3곳이었다** — 목록 초기 로드 외에 ① URL `?conversation=` 로 자동 선택된 대화 ② 대화 클릭 시 `result.success === false` 도 조용히 무시되고 있었다(throw 만 토스트를 띄웠다). 두 경로는 서버가 준 사유를 토스트로 알리도록 보완. 단위 테스트 5건 신규(정상 빈 상태 보존 특성화 포함).

---

### #011. PBL 결과 화면에 공유 토글 누락 — 갤러리에 올릴 수단 자체가 없음

> ⚠️ **2026-07-30 정정 — 아래 원문의 대상 화면이 틀렸다.**
> 원문은 _"갤러리 > PBL 카드 상세에 토글이 없다(같은 위치의 로드맵 상세에는 있다)"_ 라고 적었으나,
> 실측하면 **갤러리 상세에는 로드맵도 토글이 없다.** 원문이 "로드맵에는 있다"고 본 것은 갤러리가
> 아니라 **컨설턴트 > 프로젝트 > 로드맵 결과 화면**이다.
> 갤러리 상세에 토글이 없는 것은 결함이 아니다 — 갤러리는 **이미 공유된 것을 남이 보는 곳**이라
> 거기서 끄면 보고 있던 페이지가 사라지고, 애초에 "켜기"는 불가능한 반쪽 컨트롤이 된다.
> **진짜 결함은 PBL 결과 화면에 공유 수단이 아예 없다는 것**이고(로드맵 결과 화면에는 있다),
> 그래서 PBL 보고서는 갤러리에 올릴 방법 자체가 없었다.
> 또 _"UI 만 누락"_ 도 부정확했다 — `ShareToggle` 이 로드맵 전용으로 하드코딩돼 있었다.
...
- **상태 변경:** 🔴 OPEN → 🟢 RESOLVED (2026-07-30) — **PBL 결과 화면**(`PBLResultClient`)에 로드맵과 동일한 규칙으로 공유 컨트롤을 추가했다: FINAL 버전에서만, 컨설턴트는 토글 / Ops 는 읽기 전용 배지(`shareControl` capability 신설). `ShareToggle` 에는 `track` prop 을 추가해 `LikeButton` 과 같은 단일 prop 분기 방식으로 일반화했고, **기본값을 `ROADMAP` 으로 둬 기존 로드맵 호출부는 무변경**이다(그 기본값을 특성화 테스트로 고정). 사용자 정책 결정: _"PBL 도 공유가 되어야 한다."_ 문구는 "다른 컨설턴트가 이 PBL 보고서를 열람하고 활용할 수 있습니다."로 승인받았다. `GalleryPBLDetailContent.tsx` 는 **손대지 않았다**(위 정정 참조). 단위 테스트 8건 신규.
...
- **상태 변경 (2026-07-30, PR #147):** 🔴 OPEN → 🟢 **RESOLVED**. 다만 **전제 2개가 실제와 달랐다.**
  1. **현재 발생하지 않는 결함이었다.** 알림을 만드는 호출처 **10곳 전부** `link` 를 넣고 있었다
     (전수 확인). 즉 `link` 가 null 인 알림은 앱 경로로 생성되지 않는다. 위험은 "**나중에** 새 알림
     유형을 추가하며 빠뜨리는 것"이었다 → **근본 수정은 생성 시점 차단**이다:
     `createNotificationSchema.link` 를 필수(`.min(1)`)로, `CreateNotificationParams`·
     `AdminNotificationParams` 의 `link` 를 필수 타입으로 바꿔 **컴파일이 막게** 했다.
     (`createNotificationForAdmins` 는 Zod 검증을 거치지 않으므로 타입이 유일한 방어 —
     그 사실을 테스트로 고정해 뒀다.)
  2. **`/notifications` 로 보낼 수 없다.** 그 경로에는 `actions.ts` 만 있고 **페이지가 없어**
     이동하면 404 다. → 화면 방어는 **안내 토스트**로 했다(`showInfoToast` 신규 —
     에러가 아닌 안내에 `showErrorToast` 를 쓰면 문제가 생긴 줄 오해하므로).
- **기술 근거:**
...
- **상태 변경 (2026-07-31):** 🔴 OPEN → 🟢 **RESOLVED** (PR #152).
  `showSuccessToast`/`showErrorToast` 로 치환. **화면 동작은 변하지 않는다** — 두 래퍼 모두 인자가
  1개일 때 `toast.success(t, undefined)` / `toast.error(t, undefined)` 를 그대로 호출한다(`toast.ts:41,50`).
  ⚠️ 문서에 없던 동반 수정: 같은 폴더 `.test.tsx` 가 `vi.mock('sonner')` 로 직접 모킹하고 있어
  `@/lib/utils/toast` 모킹(위임 변수 방식, `LikeButton.test.tsx` 선례)으로 함께 바꿨다.
  성공 토스트 단언도 추가해 **양쪽 경로 모두** 래퍼를 거치는지 고정했다.
...
- **상태 변경 (2026-07-31):** 🔴 OPEN → 🟢 **RESOLVED** (PR #152).
  - ⚠️ **범위 정정 — 이 결함은 양방향이었다.** 본 보고서는 roadmap 쪽만 지목했으나
    `pbl/page.tsx` 도 로드맵 트랙을 `/pbl` 에서 똑같이 무사유로 튕긴다. **두 곳 모두** 수정했다.
  - 두 리다이렉트가 목적지에 `?trackMismatch=1` 을 실어 보내고, 프로젝트 상세가 그 경우에만
    안내 배너(`_components/TrackMismatchNotice.tsx`)를 띄운다. 같은 방식이 앱에 이미 있다
    (`login?password-changed=1` → 로그인 화면 안내).
  - 배너 문구·링크는 `lib/utils/project-track.ts` 헬퍼(`trackShortLabel`·`projectDetailHref`·
    `primaryActionLabel`)로 만든다 — 헤더의 트랙 뱃지·액션 버튼과 **같은 출처**라 명칭이 어긋나지 않는다.
  - ✅ **운영관리자 쪽은 범위 밖**: `ops/projects/[id]/roadmap|pbl` 에는 트랙 조건 자체가 없어
    튕기지 않는다(빈 데이터로 렌더). 감사 문서가 이를 언급하지 않았으나 실측으로 확인했다.
  - 안전망: 신규 `roadmap/page.test.tsx`·`pbl/page.test.tsx`(리다이렉트 목적지) +
    `TrackMismatchNotice.test.tsx`(문구·링크) + `page.test.tsx`(배너 노출 조건).
    **배너 조건을 뒤집는 결함 주입으로 4건이 실패함을 확인**해 감시력을 검증했다.
...
- **상태 변경 (2026-07-30, PR #147):** 🔴 OPEN → 🟢 **RESOLVED**.
  마이그 `077_add_notifications_type_index.sql` 로 `(user_id, type, created_at DESC)` 복합 인덱스를
  추가했다(정렬 방향까지 담아 필터 + 최신순을 한 번에 처리). 부분 인덱스가 아니라 복합 인덱스를
  택한 이유: 운영관리자 탭이 `type` 을 **여러 값으로** 필터하므로(`OPS_NOTIFICATION_TABS` 4종)
  `type != 'message'` 한 조건에 맞춘 부분 인덱스보다 범용성이 크다.
...
- **상태 변경:** 🔴 OPEN → 🟢 RESOLVED (2026-07-30) — 마이그 `079_add_audit_logs_composite_indexes.sql` 로 두 복합 인덱스 추가(052 가 `action` 에 적용한 패턴과 동일 형태). 기존 단일 인덱스는 다른 조회 경로가 쓰므로 삭제하지 않았다. 조회 코드는 무변경.
  ⚠️ **계획서의 마이그 번호 `070` 은 이미 `070_audit_actions_pr5.sql` 이 점유** — 실제 번호는 `079` 다(#016 이 `069`→`077` 로 겪은 것과 같은 drift).
  ⚠️ 실제 서버 필터는 문서가 적은 2개가 아니라 **5개**(action·target_type·actor_user_id·시작일·종료일)이며, `action` 은 052 로 이미 커버된다. 화면 상단 검색창은 **서버로 가지 않는 클라이언트 필터**라 인덱스와 무관하다.
  ⚠️ **운영 DB 반영은 별도** — 이 PR 머지만으로는 운영에 인덱스가 적용되지 않는다(로컬은 적용·검증 완료).
...
- **상태 변경 (2026-07-30):** 🔴 OPEN → ⚪ **결함 아님 (의도된 동작)**.
  정책 결정을 사용자에게 확인한 결과 **"초안이어도 PDF·HWPX 등 파일 다운로드는 가능해야 한다"**.
  따라서 계획서의 옵션 A(차단)·B(워터마크)를 모두 배제하고 **현 동작을 유지**한다. 코드 변경 없음.
