...
| 2026-07-29 | 재배정 차단 안내 (PR #138)     | #006 RESOLVED                                   |
...
| #006 | **P2** ★★★★ | 운영관리 | 운영관리 > 프로젝트 상세 > 컨설턴트 재배정 | 인터뷰 완료 이후 단계에서 재배정 시 DB 에러로 차단 | 🟢 RESOLVED |
...
- **상태 변경:** 🔴 OPEN → 🟢 RESOLVED (2026-07-29, PR #138) — 두 선택지 중 **UI 차단(옵션 B)** 을
  사용자가 선택했다. 화면(`AssignmentTabSection.tsx`)이 `FINALIZED` 만 제외하던 것을 RPC 허용 목록과
  같은 규칙으로 맞추고(신규 `REASSIGNABLE_STATUSES`·`canReassignConsultant`), 막힌 상태에는 사유를
  표시한다 — 인터뷰·초안 단계는 "인터뷰가 완료되어 담당 컨설턴트를 변경할 수 없습니다.",
  최종 확정은 "최종 확정된 프로젝트는 담당 컨설턴트를 변경할 수 없습니다."(기존에는 버튼만 없고
  이유가 침묵했다). **RPC·마이그레이션은 무변경.**
  > ⚠️ **확장(옵션 A)을 배제한 근거 — 나중에 재검토할 경우 반드시 볼 것.**
  > RPC 5단계가 `SET status = 'ASSIGNED'` 를 무조건 실행한다. 허용 상태만 넓히면 인터뷰·초안이 있는
  > 프로젝트가 상태만 "배정됨"으로 **후퇴**해 목록 뱃지·다음 단계 가드가 어긋나는 새 데시싱크가 생긴다.
  > 확장하려면 상태 보존 로직을 함께 설계해야 한다. (참고: 컨설턴트 접근 판정은 `assigned_consultant_id`
  > 기준이라 재배정해도 인터뷰 기록은 새 컨설턴트가 이어받는다 — 데이터 유실 우려는 없다.)
  >
  > **남은 제약:** 인터뷰 완료 후 담당자 교체가 실무에서 필요해지면 옵션 A 작업이 별도로 필요하다.
