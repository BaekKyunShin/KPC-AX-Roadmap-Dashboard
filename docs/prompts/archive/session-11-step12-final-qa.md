...
=== 필수 사전 정독 ===
> 계획서에서 해당 섹션은 `grep -n '^## 0\.\|^### Step 12:\|^## 6\.\|^## 7\.\|^## 8\.' docs/plans/archive/2026-04-14-official-form-alignment.md` 로 정확한 줄 위치 찾아 Read.
...
**Task 3.6 (Session 1~10 전체 워크플로우 Playwright MCP 회귀 감사 — 신규)**:
   ...
   - 참고 입력 문서 (반드시 정독 — 각 Step 의 "성공 지표" 기반으로 시나리오 추출 / **총 13 문서**):
     - 마스터 계획서 `docs/plans/archive/2026-04-14-official-form-alignment.md` §4 Step 1~11 각각 (1개 파일 안의 11 Step 섹션)
     - `docs/prompts/archive/session-00-overview.md` (전체 개요)
     - `docs/prompts/archive/session-01-step1-setup.md` ~ `session-09-step10-pbl-hwpx.md` (9개 파일)
