...
=== 필수 사전 정독 ===
> 계획서 해당 섹션의 정확한 줄 위치는 `grep -n '^## 0\.\|^### 3-4\.\|^### Step 3:\|^### Step 7:' docs/plans/archive/2026-04-14-official-form-alignment.md` 로 헤더 재확인.
...
0. **[필수] 전체 회귀 테스트 수행** — 모든 구현이 끝난 뒤 기존 기능 회귀 방지를 위해 반드시 실행. 건너뛰기 금지.
   ...
   - `npm run test:e2e` (E2E 전체 — 해당되는 경우)
   - pytest (Python 측 테스트 — 해당되는 경우)
