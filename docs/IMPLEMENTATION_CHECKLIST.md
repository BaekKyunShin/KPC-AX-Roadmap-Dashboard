...
- [ ] `README.md` 작성 (로컬 실행 방법, 환경변수, Supabase 설정)
...
- [x] `case_assignments` 테이블 (배정 이력 + 사유)
...
- [x] 컨설턴트는 배정된 케이스만 접근 가능
- [x] OPS_ADMIN은 모든 케이스 조회/관리 가능
...
## Phase 3: 케이스 생성 및 자가진단 ✅ 완료

### 3.1 케이스 생성 (OPS_ADMIN)
...
- [x] 후보별 결과:
  ...
  - [x] 근거 (rationale)
- [x] 기본값 N=3
- [x] 케이스 상태: MATCH_RECOMMENDED로 변경

### 4.2 배정 (OPS_ADMIN)
- [x] 추천 후보 중 최종 배정 선택
- [x] 배정 사유 필수 입력
- [x] 케이스 상태: ASSIGNED로 변경
- [x] 배정 이력 저장 (case_assignments)
- [x] 감사로그 기록
...
### 5.2 접근 권한
- [x] 배정된 케이스만 접근 가능 (RLS)
- [x] 자가진단 결과는 조회만 가능 (수정 불가)
- [x] 인터뷰 입력/수정 감사로그 기록
- [x] 케이스 상태: INTERVIEWED로 변경
...
- [x] 케이스 상태: ROADMAP_DRAFTED → FINALIZED

### 6.5 수정 요청 (Revision Prompt)
- [x] 수정 요청 입력 → 새 버전 생성
- [x] 수정 요청 로그 저장
- [ ] 수동 편집 (셀/과정) + 검증 (무료/40h) - UI 추가 예정
...
### 9.2 컨설턴트 화면
- [x] 배정된 케이스 목록
- [x] 케이스 상세 (기업정보/자가진단 읽기)
- [x] 인터뷰 입력/수정
- [x] 로드맵 생성/버전 히스토리
- [x] 결과 화면 (NxM + PBL + 상세)
- [x] 수정 요청 + 수동 편집 (수정 요청만, 수동 편집은 추후)
- [x] FINAL 확정 + 다운로드

### 9.3 OPS_ADMIN 화면
- [x] 사용자 승인/정지
- [x] 케이스 관리 (생성/자가진단/매칭/배정)
- [ ] 자가진단 템플릿 관리 (추후)
- [x] 감사로그/사용량/쿼터 대시보드
- [x] 로드맵 열람 (읽기 전용)
...
- [x] OPS_ADMIN이 기업정보+자가진단 입력하여 케이스 생성 가능
- [x] 자가진단+기업정보 기반 매칭 Top-N 제시 + 배정/변경(사유 기록) 가능
- [x] 승인 컨설턴트는 배정된 케이스만 접근 가능 (RLS)
