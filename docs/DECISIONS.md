...
## ADR-032: 코드 계층 성능 최적화 중단

**날짜**: 2026-08-01

**결정**: 2026-07-31 성능 감사가 제시한 코드 계층 개선 항목(미들웨어 인증 중복 제거·로드맵 로더 직렬 대기 정리·캐시 우회 페이지 정리·RLS 헬퍼 InitPlan 래핑·결과 화면 탭 코드 분할)을 **모두 진행하지 않는다.** 이미 시도한 탭 코드 분할은 원복했다.

**이유:**

- 감사가 지목한 실제 병목은 **인프라 3대륙 분산**이었고, 서울 리전 이전(`docs/decisions/2026-08-01-seoul-region-migration.md`)으로 해결됐다 — 동적 페이지 응답 0.51~1.24초 → 0.12~0.17초
- 코드 계층 항목은 전부 "서버 왕복 횟수·바이트를 줄이자"는 것인데, 왕복 1회 비용이 **~200ms → ~2ms** 로 떨어지면서 절약분이 수백 ms → 수 ms 로 축소됐다. 감사의 "효과 높음" 평가는 뭄바이 DB 기준이었다
- 이전 후 CI 실측: 인증 페이지 **TTFB 95~113ms · LCP 160~348ms**. 깎을 여지가 사실상 없다
- 유일하게 실제로 시도한 탭 코드 분할은 **역효과**로 실측됐다 — 초기 진입 16ms 이득 vs 탭 전환 59ms → 401ms (`docs/decisions/2026-08-01-result-tabs-no-code-split.md`)
- 대상 중 인증 관련부(미들웨어)는 과거 무한 리다이렉트 회귀 이력이 있어 위험 대비 이득이 맞지 않는다

**함께 진행한 것** (속도와 무관하지만 유지):

- 중복 인덱스 정리·FK 커버링 인덱스 추가 (#163) — 현 데이터 규모에서 체감 변화 없음, 위생·증가 대비
- 성능 예산을 실측 기준으로 조임 (#165) — 기존 임계값이 실측의 10~20배라 어떤 회귀도 잡지 못했다

**재검토 조건**: 사용자가 특정 화면을 지목해 느리다고 보고하는 경우. 그때도 **먼저 그 화면을 실측**해 원인이 서버 왕복임을 확인한 뒤 손댈 것 — 미검증 가정으로 착수하면 이번 탭 분할처럼 역효과가 난다. 그 외 신호: 데이터가 수 배 증가, 동시 사용자 증가, 성능 예산이 CI 에서 깨지기 시작.
