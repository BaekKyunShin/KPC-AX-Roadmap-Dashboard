...
## ADR-032: 운영 인프라 서울 리전 정렬 (Supabase 이전 + Vercel 함수 리전)

**날짜**: 2026-08-01

**결정**: 운영 Supabase 프로젝트를 뭄바이(ap-south-1)에서 **서울(ap-northeast-2)로 신규 생성·이전**하고, Vercel 함수 리전을 미국(iad1)에서 **서울(icn1)로 고정**한다.

- 운영 프로젝트: `axflsiffdbkitptgpavv`(뭄바이) → `eabynvjnadzuiddjnama`(서울, `roadmap-dashboard-kr`)
- `vercel.json` 에 `"regions": ["icn1"]` 명시 (미지정 시 기본값이 iad1)

**배경**: "페이지 이동·버튼 클릭이 전반적으로 느리다"는 문제로 코드베이스 전수 성능 감사를 수행한 결과, 데이터량(DB 23MB·프로젝트 74건)이나 쿼리 품질이 아니라 **인프라가 3대륙에 분산된 것**이 주된 원인이었다. 사용자(서울) → 함수(미국) → DB(인도) 구조에서 함수↔DB 왕복이 ~200ms 이고, 인증 확인을 포함해 페이지 1회 렌더에 6~8회 왕복이 누적됐다.

**이유**:

- 애플리케이션 코드는 이미 최적화 상태 — N+1 쿼리 0건, `.in()` 배치·`Promise.all` 병렬화 광범위 적용, 무거운 라이브러리 지연 로드 완료. 코드로 줄일 여지보다 네트워크 지연이 압도적이었다
- 리전 정렬은 기능·UI 변경이 전혀 없으면서 효과가 가장 크다
- 실측: 동적 페이지 응답 **0.51~1.24초 → 0.12~0.17초**, 앱↔DB 왕복 **~200ms → ~2ms**(동일 리전)

**대안 검토**:

- _Supabase 대시보드의 "Restore to another project"_: 동일 리전 전용이라 리전 변경에 사용 불가 (공식 문서 확인)
- _신규 DB 에 `supabase db push` 로 마이그레이션 재적용_: 운영 `schema_migrations` 가 MCP `apply_migration` 기반 타임스탬프 버전과 순차 번호로 혼재해 있고, 019(비멱등 `ALTER PUBLICATION`)·062/065(`ON CONFLICT DO UPDATE` 로 버킷 설정 덮어씀) 함정이 있어 채택하지 않음. **CLI 전체 덤프/복원**으로 현 상태를 그대로 복제
- _뭄바이 유지 + 함수만 bom1_: 중간 단계로 실제 적용했고 0.22~0.49초로 개선됐으나, 사용자·DB 모두 서울인 편이 우월

**트레이드오프**:

- 전 사용자 1회 재로그인 필요 (신규 프로젝트는 JWT 서명 키가 달라 기존 세션 무효)
- 컷오버 중 서비스 쓰기 중단 (실제 약 40분). 구 DB 를 `default_transaction_read_only = on` 으로 강제 동결해 유입 쓰기가 조용히 소실되지 않고 실패하도록 처리
- Storage 객체·auth/storage RLS 정책·pg_cron 잡·Realtime publication 은 DB 덤프에 포함되지 않아 별도 이관 필요

**운영 제약 (기록)**: Supabase **Pro 플랜 프로젝트는 pause 불가**("Please downgrade it to free-tier first"). 구 프로젝트 과금을 멈추려면 삭제만이 방법이므로, 검증 유예 기간 동안은 이중 과금(하루 ~450원)을 감수한다.

**재검토 조건**: 해외 사용자 비중이 유의미해지는 경우(멀티 리전·읽기 복제본 검토), 또는 Vercel/Supabase 가 서울 리전에서 장애·용량 제약을 보이는 경우
