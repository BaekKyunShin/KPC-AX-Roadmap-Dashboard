...
test.describe.skip('컨설턴트 인터뷰 (legacy 6스텝 — OFA-12에서 제거)', () => {
  ...
  test('다음/이전 버튼 동작 — 스텝 진행 확인', async ({ consultantPage: page }) => {
    ...
    await expect(page.locator('main label').filter({ hasText: '인터뷰 날짜' }).first()).toBeVisible();

    // "다음" 버튼 클릭 → 2단계로 이동
    await page.getByRole('button', { name: '다음' }).click();
    // 스텝 전환 후 "인터뷰 날짜" 필드가 사라지는 것으로 2단계 진입 확인
    await expect(page.locator('main label').filter({ hasText: '인터뷰 날짜' }).first()).not.toBeVisible({ timeout: 5_000 });
    ...
    await expect(page.locator('main label').filter({ hasText: '인터뷰 날짜' }).first()).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('1단계: 필수 필드 (날짜, 참석자명) 확인', async ({ consultantPage: page }) => {
    ...
    // 필수 표시 (*) 확인
    await expect(page.locator('main label').filter({ hasText: '인터뷰 날짜' }).first()).toBeVisible();
    await expect(page.locator('main label').filter({ hasText: '인터뷰 참석자' }).first()).toBeVisible();
  });
