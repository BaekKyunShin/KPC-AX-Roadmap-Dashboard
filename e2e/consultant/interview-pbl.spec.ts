...
test.describe('컨설턴트 PBL 인터뷰 V2 (양식 2:1 정합 9 스텝 + STT 첨부 1)', () => {
  ...
  test('Ⅱ-1-가 기업 경영 이슈 스텝 이동 및 textarea 편집', async ({ consultantPage: page }) => {
  ...
  test('Ⅱ-1-나 조직 및 주요 업무 스텝 — OrganizationTree 렌더', async ({
    ...
    await expect(page.getByRole('heading', { name: '조직 및 주요 업무', level: 2 })).toBeVisible();
    // OrganizationTree 위젯의 "루트 노드 추가" 버튼
    await expect(page.getByRole('button', { name: '루트 노드 추가' })).toBeVisible();
  });

  test('Ⅱ-3-가 HRD이음 PDF 업로드 UI 렌더 (accept=pdf)', async ({ consultantPage: page }) => {
    test.skip(!isPblAvailable || !interviewUrl, 'PBL 인터뷰 URL 없음');
    const getErrors = setupConsoleErrorCheck(page);
  ...
  test('Ⅲ-3·4 훈련대상·AI수준 — 4등급 radio 2조 렌더', async ({ consultantPage: page }) => {
    test.skip(!isPblAvailable || !interviewUrl, 'PBL 인터뷰 URL 없음');
    await page.goto(interviewUrl!);
    await page.waitForLoadState('networkidle');
    ...
    await expect(page.getByRole('radiogroup', { name: '예상 AI역량 수준' })).toBeVisible();
  });
