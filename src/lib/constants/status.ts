import type { CaseStatus, RoadmapVersionStatus, UserStatus } from '@/types/database';

/**
 * 케이스 상태 설정 (OPS 관리자용 - 전체 상태)
 */
...
/**
 * 케이스 상태 설정 (컨설턴트용 - 배정 이후 상태)
 */
export const CONSULTANT_CASE_STATUS_CONFIG: Record<string, { label: string; color: string }> = {
 ...
 * 상태에 따른 배지 정보 반환
 */
export function getCaseStatusBadge(status: CaseStatus): { label: string; color: string } {
  return CASE_STATUS_CONFIG[status] || { label: status, color: 'bg-gray-100 text-gray-800' };
}

/**
 * 컨설턴트용 상태 배지 정보 반환 (인터뷰 여부 고려)
 */
export function getConsultantCaseStatusBadge(
  status: string,
  ...
  return CONSULTANT_CASE_STATUS_CONFIG[status] || { label: status, color: 'bg-gray-100 text-gray-800' };
}
