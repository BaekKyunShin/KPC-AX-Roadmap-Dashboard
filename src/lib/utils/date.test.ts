...
describe('formatTimeKR', () => {
  ...
  it('Date 객체도 처리', () => {
    // 2026-04-19T15:00:00Z → KST 다음날 00:00 (자정). ICU 구현별 24:00 표기
    // 차이를 막기 위해 hourCycle: 'h23' 을 date.ts 에서 명시.
    expect(formatTimeKR(new Date('2026-04-19T15:00:00Z'))).toBe('00:00');
  });

  // ICU 환경별 자정 경계 회귀 방지 — 로컬(macOS)과 CI(Ubuntu) 모두 "00:00" 보장
  it('KST 자정은 항상 "00:00" 으로 반환한다 (24:00 아님)', () => {
    // 2026-04-19T15:00:00Z = KST 2026-04-20 00:00
    expect(formatTimeKR('2026-04-19T15:00:00Z')).toBe('00:00');
    // 2026-04-19T15:00:00.000Z 의 ms 차이도 자정 유지
    expect(formatTimeKR('2026-04-19T15:00:00.500Z')).toBe('00:00');
  });

  it('KST 낮 12시·오후 시각도 24시간제로 반환한다', () => {
    // UTC 03:00 = KST 12:00
    expect(formatTimeKR('2026-04-19T03:00:00Z')).toBe('12:00');
    // UTC 11:30 = KST 20:30
    expect(formatTimeKR('2026-04-19T11:30:00Z')).toBe('20:30');
  });
});
