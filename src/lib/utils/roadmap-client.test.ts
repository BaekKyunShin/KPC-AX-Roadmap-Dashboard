...
describe('buildRoadmapMatrixFromCourses', () => {
  it('같은 target_task 과정들이 하나의 row로 그룹화', () => {
    const courses = [
  ...
  it('task_id가 task_1, task_2 순차 증가', () => {
    const courses = [
  ...
  it('빈 courses 배열이면 빈 rows 배열 반환', () => {
    const rows = buildRoadmapMatrixFromCourses([]);
    expect(rows).toEqual([]);
  });

  it('한 task에 같은 레벨 과정 여러 개', () => {
    const courses = [
...
describe('validateCourseClient', () => {
  ...
  it('recommended_hours가 0 이하이면 에러', () => {
  ...
  it('도구의 free_tier_info 누락 시 경고 (isValid는 true)', () => {
...
describe('validateRoadmapClient', () => {
  ...
  it('하나라도 에러 있으면 isValid: false, 모든 에러/경고 수집', () => {
    const courses = [
