...
export const guideDataSchema = z.object({
  ...
  key_points: z.array(guideKeyPointSchema).min(1, '핵심 파악 포인트가 1개 이상 필요합니다.'),
  questions: z.array(guideQuestionSchema).min(1, '추천 질문이 1개 이상 필요합니다.'),
  cautions: z.array(z.string().min(1)).min(1, '주의사항이 1개 이상 필요합니다.'),
});
