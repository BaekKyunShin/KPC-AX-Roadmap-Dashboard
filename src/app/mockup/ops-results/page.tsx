...
const PROJECTS: ProjectRow[] = [
  { company: '㈜한국전자', consultant: '김컨설턴트', status: 'COMPLETED', docsSubmitted: 4, docsTotal: 4, submittedAt: '2026-04-10' },
  { company: '대한물산㈜', consultant: '이컨설턴트', status: 'FINALIZED', docsSubmitted: 2, docsTotal: 4, submittedAt: null },
  { company: '미래테크', consultant: '김컨설턴트', status: 'FINALIZED', docsSubmitted: 0, docsTotal: 4, submittedAt: null },
  { company: '글로벌솔루션', consultant: '박컨설턴트', status: 'COMPLETED', docsSubmitted: 4, docsTotal: 4, submittedAt: '2026-04-08' },
  { company: '㈜스마트팩토리', consultant: '이컨설턴트', status: 'COMPLETED', docsSubmitted: 4, docsTotal: 4, submittedAt: '2026-04-07' },
  { company: '청운산업', consultant: '김컨설턴트', status: 'FINALIZED', docsSubmitted: 3, docsTotal: 4, submittedAt: null },
  { company: '㈜넥스트AI', consultant: '박컨설턴트', status: 'COMPLETED', docsSubmitted: 4, docsTotal: 4, submittedAt: '2026-04-05' },
  { company: '한빛전기', consultant: '이컨설턴트', status: 'COMPLETED', docsSubmitted: 4, docsTotal: 4, submittedAt: '2026-04-03' },
  { company: '동방테크놀로지', consultant: '김컨설턴트', status: 'FINALIZED', docsSubmitted: 1, docsTotal: 4, submittedAt: null },
  { company: '삼성코리아', consultant: '박컨설턴트', status: 'COMPLETED', docsSubmitted: 4, docsTotal: 4, submittedAt: '2026-04-01' },
];

const CONSULTANTS = ['전체', '김컨설턴트', '이컨설턴트', '박컨설턴트'] as const;

/* ------------------------------------------------------------------ */
/*  Consultant avatar colors                                           */
/* ------------------------------------------------------------------ */

const CONSULTANT_COLORS: Record<string, { bg: string; text: string }> = {
  '김컨설턴트': { bg: 'bg-blue-100', text: 'text-blue-700' },
  '이컨설턴트': { bg: 'bg-purple-100', text: 'text-purple-700' },
  '박컨설턴트': { bg: 'bg-teal-100', text: 'text-teal-700' },
};

const DEFAULT_AVATAR_COLOR = { bg: 'bg-gray-100', text: 'text-gray-600' };
...
function filterRows(
  ...
  if (consultant !== '전체') {
    filtered = filtered.filter((r) => r.consultant === consultant);
  }
  if (query.trim()) {
...
function ConsultantCell({ name }: { name: string }) {
  const initial = name.charAt(0);
  const colors = CONSULTANT_COLORS[name] ?? DEFAULT_AVATAR_COLOR;
  return (
    <div className="flex items-center gap-2.5">
      <div className={cn('flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold', colors.bg, colors.text)}>
        {initial}
      </div>
...
export default function OpsResultsMockupPage() {
  const [activeTab, setActiveTab] = useState<FilterTab>('전체');
  const [selectedConsultant, setSelectedConsultant] = useState('전체');
  const [searchQuery, setSearchQuery] = useState('');

  const totalCount = 45;
  const unsubmittedCount = 18;
  const submittedCount = 27;

  const rows = useMemo(
    () => filterRows(PROJECTS, activeTab, selectedConsultant, searchQuery),
    [activeTab, selectedConsultant, searchQuery],
  );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
      {/* Header */}
      <PageHeader title="결과물 관리" description="전체 프로젝트의 서류 제출 현황을 관리합니다." />
      ...
      {/* Filter tabs + Consultant dropdown + Search */}
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        {/* Left: Tabs + Consultant filter */}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="relative">
            <select
              ...
              className="h-9 cursor-pointer appearance-none rounded-md border border-gray-200 bg-white pl-3 pr-9 text-sm font-medium text-gray-600 transition-colors hover:border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
            >
              {CONSULTANTS.map((c) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-gray-50/80 text-left text-xs uppercase tracking-wider text-gray-500">
                  ...
                  <th className="whitespace-nowrap px-6 py-3 font-medium">담당 컨설턴트</th>
                ...
                {rows.map((row) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-6 py-4">
                      <ConsultantCell name={row.consultant} />
                    </td>
                    <td className="whitespace-nowrap px-6 py-4">
