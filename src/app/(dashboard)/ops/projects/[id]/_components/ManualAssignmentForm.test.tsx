...
let _mockOnSelect: ((consultant: ConsultantCandidate | null) => void) | null = null;
...
/** "배정하기" 클릭 후 AlertDialog 의 "배정 확인" 까지 누르는 흐름 */
async function clickAssignAndConfirm(user: ReturnType<typeof userEvent.setup>) {
  await user.click(screen.getByRole('button', { name: '배정하기' }));
  const confirm = await screen.findByRole('button', { name: '배정 확인' });
...
describe('ManualAssignmentForm', () => {
  ...
  describe('초기 렌더링', () => {
    ...
    it('초기에 에러 메시지가 없다', () => {
      renderForm();
      expect(screen.queryByText('배정할 컨설턴트를 선택하세요.')).not.toBeInTheDocument();
    });

    it('currentAssignment 가 없으면 재배정 안내 띠가 노출되지 않는다', () => {
      renderForm();
      expect(screen.queryByText(/현재 배정:/)).not.toBeInTheDocument();
    });
  });
  ...
  describe('비가역 액션 사전 차단 (AlertDialog)', () => {
    it('"배정하기" 클릭 시 즉시 assignConsultant 가 호출되지 않고 다이얼로그가 노출된다', async () => {
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
      ...
      expect(await screen.findByRole('button', { name: '배정 확인' })).toBeInTheDocument();
    });

    it('다이얼로그 "취소" 클릭 시 assignConsultant 호출 0회', async () => {
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
      await enterReason(user, '이 컨설턴트는 해당 분야에 전문 경험이 풍부합니다.');
    ...
    it('신규 배정(currentAssignment=null)일 때 다이얼로그 라벨이 "<이름>컨설턴트를 이 프로젝트에 배정하시겠습니까?" 로 표시된다', async () => {
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
      await enterReason(user, '이 컨설턴트는 해당 분야에 전문 경험이 풍부합니다.');
    ...
    it('재배정(currentAssignment 있음)일 때 안내 띠 + 다이얼로그 워딩이 변경된다', async () => {
      const user = userEvent.setup();
      renderForm({
        currentAssignment: { id: 'a-1', name: '박기존', email: 'a@test.com' },
      });
      // 안내 띠
      expect(screen.getByText(/현재 배정:/)).toBeInTheDocument();
      expect(screen.getByText('박기존')).toBeInTheDocument();
  ...
  describe('제출 (다이얼로그 확인 후)', () => {
    it('다이얼로그 "배정 확인" 클릭 시 assignConsultant 가 호출되고 router.refresh 가 실행된다', async () => {
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
    ...
    it('제출 시 FormData 에 올바른 값이 전달된다', async () => {
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
      await enterReason(user, '이 컨설턴트는 해당 분야에 전문 경험이 풍부합니다.');
    ...
    it('제출 실패 시 에러 메시지가 표시된다', async () => {
      ...
      await clickAssignAndConfirm(user);
      await waitFor(() => {
        expect(screen.getByText('이미 배정된 프로젝트입니다.')).toBeInTheDocument();
      });
    });

    it('제출 실패 시 에러 메시지가 없으면 기본 메시지를 표시한다', async () => {
      mockAssignConsultant.mockResolvedValue({ success: false });
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
      await enterReason(user, '이 컨설턴트는 해당 분야에 전문 경험이 풍부합니다.');
  ...
  describe('에러 닫기', () => {
    it('에러 메시지의 닫기 버튼을 클릭하면 에러가 사라진다', async () => {
      mockAssignConsultant.mockResolvedValue({ success: false, error: '배정 실패' });
      const user = userEvent.setup();
      renderForm();
      await selectConsultant(user);
      await enterReason(user, '이 컨설턴트는 해당 분야에 전문 경험이 풍부합니다.');
