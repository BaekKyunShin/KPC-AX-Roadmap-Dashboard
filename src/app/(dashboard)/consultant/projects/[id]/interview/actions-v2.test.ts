...
async function mockCachedAuth({
  ...
  // 타입 단언으로 Supabase SDK User / DB Profile 전체 필드를 만족시키지 않고도
  // 테스트에 필요한 id/email/role/status 만 제공한다.
...
describe('fetchRoadmapInterviewV2', () => {
  ...
  it('배정되지 않은 프로젝트 → null', async () => {
    await mockCachedAuth();
    serverMock.addResult({
      data: { assigned_consultant_id: 'other-user', track: 'ROADMAP' },
      error: null,
    });
  ...
  it('PBL track → null', async () => {
    await mockCachedAuth();
    serverMock.addResult({
      data: { assigned_consultant_id: USER_A, track: 'PBL' },
      error: null,
    });
  ...
  it('DB snake_case row → camelCase 복원', async () => {
    await mockCachedAuth();
    serverMock.addResult({
      data: { assigned_consultant_id: USER_A, track: 'ROADMAP' },
      error: null,
    });
    serverMock.addResult({
      data: {
    ...
    expect(r?.hrdReportPdf).toEqual({ fileName: 'a.pdf', url: 'p/a.pdf', size: 100 });
  });
});
...
describe('fetchPBLInterviewV2', () => {
  ...
  it('비배정 프로젝트 → null', async () => {
    await mockCachedAuth();
    serverMock.addResult({
      data: { assigned_consultant_id: 'other', track: 'PBL' },
      error: null,
    });
