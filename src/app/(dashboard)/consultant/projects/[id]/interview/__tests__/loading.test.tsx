 ...
 *
 * 실제 Roadmap 트랙은 9 스텝 (`ROADMAP_STEPS`), PBL 트랙은 10 스텝 (`PBL_STEPS`).
 * loading.tsx 가 트랙에 무관하게 6 스텝 고정으로 렌더하면 컨텐츠 진입 시
 * 스테퍼 자리가 3~4개 점프하므로, searchParams.track 값으로 분기되어야 함.
 */
...
describe('interview/loading.tsx', () => {
  it('searchParams 없음 → stepCount=9 (Roadmap default)', async () => {
    const ui = await InterviewLoading({ searchParams: Promise.resolve({}) });
    const { getByTestId } = render(ui);
    expect(getByTestId('mock-interview-skeleton').getAttribute('data-step-count')).toBe('9');
  });
  ...
  it('searchParams.track=ROADMAP → stepCount=9', async () => {
