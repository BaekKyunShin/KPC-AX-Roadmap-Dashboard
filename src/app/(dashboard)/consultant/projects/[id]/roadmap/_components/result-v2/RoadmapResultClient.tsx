...
// ⚠️ 탭 본문을 `next/dynamic` 으로 코드 분할하지 말 것 (#164 에서 시도 → #167 로 원복).
//    실측: 초기 진입 16ms 이득 vs **탭 전환 59ms → 401ms**. 유휴 시점 프리페치를 넣어도
//    Next.js 가 프리페치와 dynamic 로더를 별개로 취급해 클릭 시점에 청크를 새로 받는다.
//    자세한 수치와 재현 방법은 `docs/decisions/2026-08-01-result-tabs-no-code-split.md`.
...
export function RoadmapResultClient({
  ...
  useEffect(() => {
    ...
    const rafId = requestAnimationFrame(() => {
      ...
      // URL cleanup — 새로고침 시 재펼침/히스토리 누적 방지. scrollIntoView 위치
      // 보존을 위해 scroll: false 명시.
