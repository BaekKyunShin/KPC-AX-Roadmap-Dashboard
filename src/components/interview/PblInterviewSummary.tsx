      ...
      {hasStt && stt && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {(stt.추가_업무?.length ?? 0) > 0 && <SttCard title="추가 업무" items={stt.추가_업무!} />}
            {(stt.추가_페인포인트?.length ?? 0) > 0 && (
              <SttCard title="추가 페인포인트" items={stt.추가_페인포인트!} />
            )}
            {(stt.숨은_니즈?.length ?? 0) > 0 && <SttCard title="숨은 니즈" items={stt.숨은_니즈!} />}
            {isNonEmpty(stt.조직_맥락) && <SttCard title="조직 맥락" text={stt.조직_맥락} />}
            {isNonEmpty(stt.AI_태도) && <SttCard title="AI 태도" text={stt.AI_태도} />}
            {(stt.주요_인용?.length ?? 0) > 0 && <SttCard title="주요 인용" items={stt.주요_인용!} />}
          </div>
        </section>
      )}
    </div>
