-- 035_fix_quota_comparison_operator.sql
-- 목적: 쿼터 초과 메시지 정확성 수정 ("초과" → "도달")
-- 관련 이슈: 감사 #38 — >= 비교(도달 시 차단)인데 메시지는 "초과"라고 표현
-- 변경 내용: 비교 연산자(>=)는 유지하고 메시지만 "초과" → "도달"로 수정
-- 참고: >= 비교가 정확히 limit회를 허용하는 올바른 동작 (체크 후 증가 순서)
